...
function buildWordMarkup(resume: ResumeData, photoDataUrl?: string) {
  return `<!doctype html><html><head><meta charset="utf-8"/><title>${escapeHtml(resume.fullName)} Resume</title><style>body{font-family:Arial,sans-serif;color:#17353d;margin:28px;line-height:1.5}h1{font-size:28px;margin:0 0 6px}h2{font-size:12px;letter-spacing:.2em;text-transform:uppercase;color:#08606c;margin:22px 0 8px}p{margin:0 0 8px}ul{margin:8px 0 0 18px;padding:0}.meta{color:#5b6b72;margin-bottom:10px}.top{overflow:hidden}.top .copy{overflow:hidden}.photo{float:right;width:88px;height:88px;margin-left:18px;border-radius:14px;object-fit:cover;border:1px solid rgba(8,96,108,.12)}.info-row{margin-bottom:8px}.info-label{display:inline-block;min-width:110px;font-size:10px;letter-spacing:.18em;text-transform:uppercase;color:#BE481A;font-weight:700}</style></head><body><div class="top">${photoDataUrl ? `${candidateImageMarkup(photoDataUrl)}` : ""}<div class="copy"><h1>${escapeHtml(resume.fullName)}</h1><p><strong>${escapeHtml(resume.targetRole)}</strong></p><p class="meta">${escapeHtml(resume.contactLine)}</p></div></div><h2>Professional Summary</h2><p>${escapeHtml(resume.summary)}</p><h2>Core Skills</h2><p>${escapeHtml(resume.coreSkills.join(", "))}</p><h2>Professional Experience</h2>${resume.experience.map((item) => `<p><strong>${escapeHtml(item.title)}</strong> | ${escapeHtml(item.company)}</p><p class="meta">${escapeHtml([item.location, item.period].filter(Boolean).join(" | "))}</p><ul>${item.bullets.map((bullet) => `<li>${escapeHtml(bullet)}</li>`).join("")}</ul>`).join("")}<h2>Education</h2>${resume.education.map((item) => `<p><strong>${escapeHtml(item.degree || item.institution)}</strong></p><p class="meta">${escapeHtml([item.institution, item.year].filter(Boolean).join(" | "))}</p>`).join("")}${resume.certifications.length ? `<h2>Certifications</h2><p>${escapeHtml(resume.certifications.join(", "))}</p>` : ""}<h2>Personal Information</h2>${resume.personalInfo.map((item) => `<p class="info-row"><span class="info-label">${escapeHtml(item.label)}</span>${escapeHtml(item.value)}</p>`).join("")}</body></html>`;
}

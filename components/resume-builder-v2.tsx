...
function buildWordMarkup(resume: ResumeData, photoDataUrl?: string) {
  return `<!doctype html><html><head><meta charset="utf-8"/><title>${escapeHtml(resume.fullName)} Resume</title><style>body{font-family:Arial,sans-serif;color:#17353d;margin:32px;line-height:1.55}h1{font-size:28px;margin:0 0 6px}h2{font-size:12px;letter-spacing:.2em;text-transform:uppercase;color:#08606c;margin:24px 0 8px}h3{margin:0;font-size:18px}p{margin:0 0 10px}ul{margin:8px 0 0 18px;padding:0}.meta{color:#5b6b72;margin-bottom:12px}.top{display:flex;justify-content:space-between;gap:20px;align-items:flex-start}.photo{width:92px;height:92px;border-radius:14px;object-fit:cover;border:1px solid rgba(8,96,108,.12)}.info-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px 18px}.info-row{padding:10px 12px;border:1px solid rgba(8,96,108,.12);border-radius:12px}.info-label{display:block;font-size:10px;letter-spacing:.18em;text-transform:uppercase;color:#BE481A;font-weight:700;margin-bottom:4px}</style></head><body><div class="top"><div><h1>${escapeHtml(resume.fullName)}</h1><p><strong>${escapeHtml(resume.targetRole)}</strong></p><p class="meta">${escapeHtml(resume.contactLine)}</p></div>${candidateImageMarkup(photoDataUrl)}</div><h2>Personal Information</h2><div class="info-grid">${buildPersonalInfoMarkup(resume.personalInfo)}</div><h2>Professional Summary</h2><p>${escapeHtml(resume.summary)}</p><h2>Core Skills</h2><p>${escapeHtml(resume.coreSkills.join(", "))}</p><h2>Professional Experience</h2>${resume.experience.map((item) => `<p><strong>${escapeHtml(item.title)}</strong> | ${escapeHtml(item.company)}</p><p class="meta">${escapeHtml([item.location, item.period].filter(Boolean).join(" | "))}</p><ul>${item.bullets.map((bullet) => `<li>${escapeHtml(bullet)}</li>`).join("")}</ul>`).join("")}<h2>Education</h2>${resume.education.map((item) => `<p><strong>${escapeHtml(item.degree || item.institution)}</strong></p><p class="meta">${escapeHtml([item.institution, item.year].filter(Boolean).join(" | "))}</p>`).join("")}${resume.certifications.length ? `<h2>Certifications</h2><p>${escapeHtml(resume.certifications.join(", "))}</p>` : ""}</body></html>`;
}
...
function PreviewShell({ className, children }: { className?: string; children: ReactNode }) {
  return <article className={`print-preview rounded-[1.8rem] bg-white shadow-[0_18px_50px_rgba(15,47,54,0.1)] ${className || ""}`}>{children}</article>;
}
...
export function ResumeBuilder() {
  ...
  const downloadWord = () => {
    if (!resume) return;
    downloadBlob(`${resume.fullName.replace(/\s+/g, "-").toLowerCase()}-resume.doc`, buildWordMarkup(resume, form.photoDataUrl), "application/msword");
  };

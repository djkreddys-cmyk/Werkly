...
function PreviewShell({ className, children }: { className?: string; children: ReactNode }) {
  return <article className={`print-preview rounded-[1.8rem] bg-white font-[family-name:var(--font-sans)] shadow-[0_18px_50px_rgba(15,47,54,0.1)] ${className || ""}`}>{children}</article>;
}
...
function ResumePreview({ resume, template, photoDataUrl }: { resume: ResumeData; template: TemplateStyle; photoDataUrl?: string }) {
  ...
  if (template === "contrast") {
    return <PreviewShell className="overflow-hidden"><div className="grid md:grid-cols-[0.95fr_1.05fr]"><div className="bg-[linear-gradient(180deg,rgba(190,72,26,0.95),rgba(164,59,25,0.98))] p-8 text-white"><div className="flex items-center gap-4"><PhotoBadge photoDataUrl={photoDataUrl} size="md" /><div><h2 className="text-4xl font-semibold leading-none">{resume.fullName}</h2><p className="mt-2 text-lg font-semibold text-white/88">{resume.targetRole}</p></div></div><p className="mt-5 text-sm leading-7 text-white/82">{resume.contactLine}</p><div className="mt-8 rounded-[1.35rem] bg-white/10 p-5"><SectionTitle inverse>Professional Summary</SectionTitle><p className="mt-3 text-sm leading-7 text-white/84">{resume.summary}</p></div><div className="mt-6 grid gap-4 sm:grid-cols-2">{resume.personalInfo.filter((item) => item.value).slice(0, 4).map((item) => <div key={`${item.label}-${item.value}`} className="rounded-2xl border border-white/12 bg-white/6 px-4 py-3"><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-white/66">{item.label}</p><p className="mt-1 text-sm leading-6 text-white">{item.value}</p></div>)}</div></div><div className="bg-white p-8"><div className="grid gap-6 lg:grid-cols-2"><div><SectionTitle>Core Skills</SectionTitle><div className="mt-3 flex flex-wrap gap-2">{resume.coreSkills.map((skill) => <span key={skill} className="rounded-full bg-[rgba(190,72,26,0.08)] px-3 py-1 text-sm font-medium text-[var(--color-accent-strong)]">{skill}</span>)}</div></div><div><SectionTitle>Strengths</SectionTitle><ul className="mt-3 space-y-2 text-sm leading-6 text-[var(--color-ink)]">{resume.strengths.map((item) => <li key={item}>{item}</li>)}</ul></div></div><div className="mt-7"><SectionTitle>Experience</SectionTitle><div className="mt-4 grid gap-4">{resume.experience.map((item) => <div key={`${item.company}-${item.period}`} className="rounded-[1.25rem] border border-[var(--color-line)] p-5"><div className="flex flex-wrap items-baseline justify-between gap-2"><h3 className="text-xl font-semibold text-[var(--color-ink)]">{item.title}</h3><p className="text-sm text-[var(--color-muted)]">{item.period}</p></div><p className="mt-1 text-sm font-medium text-[var(--color-dark)]">{[item.company, item.location].filter(Boolean).join(" | ")}</p><ul className="mt-3 space-y-2 pl-5 text-sm leading-6 text-[var(--color-ink)]">{item.bullets.map((bullet) => <li key={bullet} className="list-disc">{bullet}</li>)}</ul></div>)}</div></div><div className="mt-7 grid gap-6 lg:grid-cols-2"><div><SectionTitle>Education</SectionTitle><div className="mt-3 space-y-3">{resume.education.map((item) => <div key={`${item.institution}-${item.year}`}><p className="text-base font-semibold text-[var(--color-ink)]">{item.degree || item.institution}</p><p className="text-sm text-[var(--color-muted)]">{[item.institution, item.year].filter(Boolean).join(" | ")}</p></div>)}</div></div>{resume.certifications.length > 0 && <div><SectionTitle>Certifications</SectionTitle><ul className="mt-3 space-y-2 text-sm leading-6 text-[var(--color-ink)]">{resume.certifications.map((item) => <li key={item}>{item}</li>)}</ul></div>}</div><div className="mt-7"><PersonalInfoPanel items={resume.personalInfo.slice(4)} /></div></div></div></PreviewShell>;
  }
...
export function ResumeBuilder({ mode = "full" }: { mode?: "full" | "modalOnly" }) {
